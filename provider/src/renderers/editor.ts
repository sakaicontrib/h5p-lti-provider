...
export default (req: CustomRequest) => (model: IEditorModel): string => `<html>
<head>
  <meta charset="UTF-8">
  <script>
  ...
  ${model.scripts
      //some scripts try to use parent object. This causes CORS errors because parent pertains to the LMS
      .filter((script) => script.indexOf('/h5p/core/js/') === 0)
...
${(req.query?.err) ? 
  `<div class="alert alert-danger alert-dismissible fade show" role="alert">
      ${req.t((req.query.err as string), { ns: 'frontend' })}
      <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="${req.t("close", { ns: 'frontend' })}"></button>
    </div>`
 :  ''}
...
<form method="post" enctype="multipart/form-data" id="h5p-content-form">
    ...
    <input id="save-h5p" type="submit" name="submit" value="${req.t((model.integration.editor?.nodeVersionId) ? 'save' : 'create' , { ns: 'frontend' })}" class="h5peditor-button-textual importance-high">
    <div class="h5peditor-button h5peditor-button-textual importance-low" role="button" onclick="window.location.href='${req.session.provider.is_launch ? '/' : '/ci'}'">${req.t('cancel' , { ns: 'frontend' })}</div>
</form>
<script>
var ns = H5PEditor;
